...
class ShareToAppModule {
  ...
  getFileNames(handler, errorHandler, url) {
    if (this.isIos) {
      ShareToApp.getFileNames(url).then(data => {
        let files = this.parseIosPayload(data);
        handler(files);
      }).catch(e => errorHandler(e));
    } else {
      ShareToApp.getFileNames().then(fileObject => {
  ...
  parseIosPayload(data) {
    const defaults = {
      ...
      fileName: null,
      mimeType: null,
      // we don't have this info for IOS, so always null
      text: null,
      weblink: null
    };
    const file = data;
...
var _default = exports.default = ShareToAppModule;
//# sourceMappingURL=ShareToApp.js.map